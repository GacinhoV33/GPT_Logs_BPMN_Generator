...
const LogHookComponent = ({
  ...
  useEffect(() => {
    async function getInitialDiagram() {
      if (diagram === "local") {
        const data = await (
          await fetch(PRODUCTION_HOST + `examples`, requestOptions)
        ).json(); // FOR TEST REQUEST
  ...
  if (diagram.length !== "local" ) {
    modeler
      ...
      .then(({ warnings }) => {
        ...
        if(apiNumber !== currentApiNumber) setCurrentApiNumber(apiNumber);

      })
      .catch((err) => {
        if(apiNumber !== currentApiNumber && apiNumber !== 0){
          setRequestStatus(requestStates.ERROR);
        }
        console.log("error", err);
      });

    if (apiNumber > 0 && requestStatus !== requestStates.CORRECT) {
